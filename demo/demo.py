...
def get_dataloader(dataset_path, cam_names, object_name):
    dataset = anodet.AnodetDataset(os.path.join(dataset_path, f"{object_name}/train/good/{cam_names}"))
...
def model_fit(model, dataloader, distributions_path, cam_names, object_name):
    ...
    torch.save(model.mean, os.path.join(distributions_path, f"{object_name}_{cam_names}_mean.pt"))
    torch.save(model.cov_inv, os.path.join(distributions_path, f"{object_name}_{cam_names}_cov_inv.pt"))
    print(f"Parameters saved at {distributions_path}")

def predict(dataset_path, distributions_path, cam_names, object_name, test_images, THRESH=13):
    images = []
    ...
    mean = torch.load(os.path.join(distributions_path, f"{object_name}_{cam_names}_mean.pt")) 
    cov_inv = torch.load(os.path.join(distributions_path, f"{object_name}_{cam_names}_cov_inv.pt"))
...
if __name__ == "__main__":
    ...
    for cam_name in cam_names:
        ...
        test_images = [
            os.path.join(dataset_path, f"{object_name[0]}/test/{anomaly[0]}/{cam_name}/000.png"),
            os.path.join(dataset_path, f"{object_name[0]}/test/{anomaly[0]}/{cam_name}/001.png"),
            os.path.join(dataset_path, f"{object_name[0]}/test/{anomaly[0]}/{cam_name}/002.png"),
            os.path.join(dataset_path, f"{object_name[0]}/test/good/{cam_name}/000.png"),
            os.path.join(dataset_path, f"{object_name[0]}/test/good/{cam_name}/001.png"),
        ]
